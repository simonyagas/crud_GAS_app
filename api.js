class API{
  ...
  getItems(sheetName, pageSize=15, page=1){
    ...
    if (!ws) throw new Error(`${sheetName} no fue encontrado`)
    const [headers, ...records] = ws.getDataRange().getValues()
    records.reverse()
    const keys = this.createKeys(headers)
    const startIndex = (page - 1) * pageSize
    const items = records.slice(startIndex, startIndex + pageSize).map(values => this.createItemObject(keys, values))
    return {
      items,
  ...
  createItem(sheetName, jsonData){
    ...
    const item = this.createItemObject(keys, newRecord)
  ...
  updateItem(sheetName, jsonData){
    ...
    if (recordIndex === -1) throw new Error(`Record ${data.id} no fue encontrado`)
    const values = keys.map((key) => {
      return data.hasOwnProperty(key) ? data[key] : null
    })
    ws.getRange(recordIndex + 2, 1, 1, values.length).setValues([values])
    const item = this.createItemObject(keys, values)
    return JSON.stringify(item)
  }
